...
from MeteorPackage import Read_AltitudeDensityTable as rd
...
class meteor:
    ...
    def ablation(self, t, y):
        ...
        # terminal is monkey patched on after initialisation
    ...
    def analyticcrater(self, t, y):
        if y[1] < 0:
            return 0
        else:
            return y[1] # will minimise when z goes to 0
    ...
    def analytical(self):
        ...
        print('work')
        datanum = self.fire()
        #eq1 = Eq(np.log(v), K * np.exp(-z/H) + C)
        z = np.linspace(z0, datanum[1][3][-1], len(datanum[1][3]))
        v = np.exp(K * np.exp(-z/H) + C)
        return v, z, datanum
    ...
    def results(self, readout=True):
        ...
        readout(bool): ensures the print out of result summary if "True"
                       and otherwise if false. (Devault value: True)
        ...
        keloss = (0.5 * yfinal[1][0] * yfinal[0][0]**2) - (0.5 * yfinal[1][lossindex] * yfinal[0][lossindex]**2)  #loss of ke
        if ablated is True:
    ...
    def fireiterate(self, r, sigma0): #m and r should be coupled
        self.sigma0 = sigma0 #CLASSWIDE USAGE MAY CAUSE ISSUES
        #ENFORCED VALUES
        v = 19.2E3
...
#def __init__(self, v, m, theta, x, r, z0=100E3, g=9.81, Rp=6400E3, sigma0=64*1E6, pa_tab=False, planet='Earth'):
#mass = (4/3) * np.pi * 8.5**3 * 3300
#mathilde = meteor(v=19.5e3, m=mass, theta=18.3/180 * np.pi, r=8.5, z0=100E3, g=9.81, Rp=6400E3, sigma0=4e6, pa_tab=False, planet="Earth")
#mathilde.plot()
#mathilde.results()
#mathilde.errorplot()

#mathilde.results()
#mathilde.energypatch(plot=True)
